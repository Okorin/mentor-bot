...
class Management:
    ...
    async def add(self, ctx, name, cycle: int, gamemode, user_type):
        if self.auth.can(ctx.message.author, config.ADD_PARTICIPANTS):
    ...
    async def associate(self, ctx, name1, name2, cycle: int, gamemode):
        if self.auth.can(ctx.message.author, config.ASSOCIATE_PARTICIPANTS):
            # load mode from customizing
            mode_id = self.gamemodes.get(gamemode)
    ...
    async def create(self, ctx, name, mentor_starts_at, mentor_ends_at,
                     ...
                     starts_at, ends_at):
        if self.auth.can(ctx.message.author, config.CREATE_CYCLES):
    ...
    async def name(self, ctx, cycle_id: int, name):
        if self.auth.can(ctx.message.author, config.RENAME_CYCLES):
    ...
    async def mentorSignups(self, ctx, cycle_id: int, start, end):
        if self.auth.can(ctx.message.author, config.RESCHEDULE_CYCLES):
    ...
    async def verify(self, ctx, *names):
        if self.auth.can(ctx.message.author, config.VERIFY_USERS):
            print(ctx)
            for name in names:
    ...
    async def delete(self, ctx, *names):
        if self.auth.can(ctx.message.author, config.DELETE_USERS):
            def check(msg):
...
def setup(client):
    client.add_cog(Management(client, db.cursor, db.auth))